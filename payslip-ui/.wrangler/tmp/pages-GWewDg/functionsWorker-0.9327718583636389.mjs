...
// ../.wrangler/tmp/bundle-6wms7t/checked-fetch.js
...
async function onRequestGet7(context) {
  ...
  try {
    try {
      const response = await fetch("http://127.0.0.1:8089/log");
      if (response.ok) {
        const data = await response.json();
        return new Response(JSON.stringify({ logs: data.logs }), {
          headers: { "Content-Type": "application/json" }
        });
      }
    } catch (e) {
    ...
    const formattedLogs = results.map(
      (row) => `[${row.timestamp}] [${row.email}] Action: ${row.action} - Description: ${row.description}`
    ).join("\n");
    return new Response(JSON.stringify({ logs: formattedLogs || "No logs recorded yet." }), {
      headers: { "Content-Type": "application/json" }
    });
  } catch (err) {
    return new Response(JSON.stringify({ error: err.message }), {
...
// ../.wrangler/tmp/pages-GWewDg/functionsRoutes-0.5427108775190854.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-6wms7t/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-6wms7t/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9327718583636389.mjs.map
