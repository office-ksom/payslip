...
// ../.wrangler/tmp/bundle-237uLl/checked-fetch.js
...
async function onRequestPost7(context) {
  ...
  if (userRole !== "approver" && userRole !== "super_admin") {
    return new Response(JSON.stringify({ error: "Only approvers can approve paybills." }), { status: 403 });
  }
  try {
    ...
    if (exists === 0) {
      return new Response(JSON.stringify({ error: "No data found for this month to approve." }), { status: 400 });
    }
    await db.prepare(`
      UPDATE monthly_earnings 
      SET is_approved = 1, approved_on = ?, approved_by = ?
      WHERE month_year = ?
    `).bind(now, userEmail, monthYear).run();
    return new Response(JSON.stringify({ success: true, approved_on: now, approved_by: userEmail }), {
      headers: { "Content-Type": "application/json" }
    });
  } catch (err) {
    return new Response(JSON.stringify({ error: err.message }), { status: 500 });
...
async function onRequestGet4(context) {
  try {
    ...
    const approvalInfo = await db.prepare(`
      ...
      WHERE month_year = ? AND is_approved = 1
      LIMIT 1
...
async function onRequest(context) {
  ...
  if (path.startsWith("/api/approve") && method !== "GET" && user.role !== "approver" && user.role !== "super_admin") {
    return forbiddenResponse();
  }
  if (method !== "GET" && (user.role === "viewer" || user.role === "approver") && !path.startsWith("/api/approve")) {
    return forbiddenResponse();
  }
  if (path === "/api/me") {
...
// ../.wrangler/tmp/pages-sEnR7b/functionsRoutes-0.8759426461015836.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-237uLl/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-237uLl/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9877719167401765.mjs.map
